...
public class ImageLab {
    ...
    private void getLayersAndDisplay() throws Exception {
        ...
        list.add(
                new Parameter<>(
                        ...
                        new KVP(Parameter.EXT, "tif", Parameter.EXT, "jpg")));//底图
        list.add(
                new Parameter<>(
                        "shape",
                        File.class,
        ...
        JMenu menu=new JMenu("add layers");
        menuBar.add(menu);
        menu.add(
                new SafeAction("add shapefile") {
                    public void action(ActionEvent e) throws Throwable {
                        System.out.println("add shp");
                        shpId++;
                    }
                });

        menu.add(
                new SafeAction("add raster") {
                    public void action(ActionEvent e) throws Throwable {
                        System.out.println("add raster");
                    }
                });

        JParameterListWizard wizard =
                new JParameterListWizard("Image Lab", "Fill in the following layers", list);
        wizard.setJMenuBar(menuBar);
        ...
        File shapeFile1 = (File) wizard.getConnectionParameters().get("shape1");
        List<File> fileList=new  ArrayList<>();
       fileList.add(shapeFile);
       fileList.add(shapeFile1);
        displayLayers(imageFile, fileList);
    }
    ...
    private void displayLayers(File rasterFile, List<File> shpFile) throws Exception {
...
//        SimpleFeatureSource shapefileSource = dataStore.getFeatureSource();
        ...
        for(int i=0;i<shpFile.size();i++){
            FileDataStore dataStore = FileDataStoreFinder.getDataStore(shpFile.get(i));
            SimpleFeatureSource shapefileSource = dataStore.getFeatureSource();
